...
HOST_PI = os.environ.get("WRO_PI_HOST", "pi@clc-wro-rm.local")
...
def run_ssh(cmd, check=True):
    full_cmd = ["ssh"] + SSH_OPTS + [HOST_PI, cmd]
...
def push():
    ...
    files = [str(f) for f in LOCAL_VISION.glob("*") if (f.is_file() or f.is_dir()) and f.name != "__pycache__"]
    cmd = ["scp", "-r"] + SSH_OPTS + files + [f"{HOST_PI}:{REMOTE_VISION}/"]
...
def pull():
    ...
    subprocess.run(["scp"] + SSH_OPTS + [f"{HOST_PI}:{REMOTE_VISION}/config.json", str(LOCAL_VISION / "config.json")])
    logs_remote = f"{HOST_PI}:{REMOTE_VISION}/logs"
    logs_local = LOCAL_VISION / "logs"
    logs_local.mkdir(parents=True, exist_ok=True)
    subprocess.run(["scp", "-r"] + SSH_OPTS + [f"{logs_remote}/*", str(logs_local)], check=False)
